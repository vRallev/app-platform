...
/**
 * `ComposePresenter` is a presenter that uses Compose core (don't confuse Compose core with Compose
 * UI, Compose UI is built on-top of Compose core) to create a [StateFlow] of models.
 ...
 *
 * Note that [ComposePresenter] itself doesn't extend the [Presenter] interface. Use
 * [launchComposePresenter] to transform a [ComposePresenter] to a [Presenter]. To use another
 * [Presenter] within a [ComposePresenter] you can inject the presenter directly and subscribe to
 * changes of [Presenter.model].
 *
 * `ComposePresenters` typically are stateless, meaning they have no properties and are not marked
 * as singletons. If no input is used, then use [Unit] for [InputT]. A typical implementation may
 * look like:
 * ```
 * @Inject
 * class MyPresenter : ComposePresenter<Unit, Model> {
 *
 *     @Composable
 ...
 * ```
 *
 * `ComposePresenters` can host and embed other child presenters. To invoke them call [present]
 ...
 * ```
 */
public fun interface ComposePresenter<InputT : Any, ModelT : BaseModel> {
