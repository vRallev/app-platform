...
import software.ralf.app.platform.listdetail.list.detail.`impl`.generated.resources.back
import software.ralf.app.platform.listdetail.list.detail.`impl`.generated.resources.character_detail_title
import software.ralf.app.platform.listdetail.list.detail.`impl`.generated.resources.character_not_available
import software.ralf.app.platform.listdetail.theme.AppTheme
import software.ralf.app.platform.renderer.ComposeRenderer

/** Material 3 renderer for a character portrait, name, age, and optional back navigation. */
@ContributesRenderer
class CharacterDetailRenderer : ComposeRenderer<CharacterDetailPresenter.Model>() {
  @Composable
  override fun Compose(model: CharacterDetailPresenter.Model, modifier: Modifier) {
    Scaffold(
        ...
        CenterAlignedTopAppBar(
          ...
          navigationIcon = {
            if (model.showBackButton) {
              IconButton(onClick = model.onBack, modifier = Modifier.testTag("detailBack")) {
                Icon(
      ...
      when (val state = model.state) {
        is CharacterDetailPresenter.State.Available ->
          CharacterContent(character = state.character, contentPadding = contentPadding)
        is CharacterDetailPresenter.State.Missing ->
          Box(
            modifier =
