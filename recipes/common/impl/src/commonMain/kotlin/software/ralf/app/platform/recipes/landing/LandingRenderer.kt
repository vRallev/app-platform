...
import app_platform.recipes.common.`impl`.generated.resources.presenter_navigation
...
class LandingRenderer : ComposeRenderer<Model>() {
  ...
  override fun Compose(model: Model, modifier: Modifier) {
    Column(
      ...
      Button(
        onClick = { model.onEvent(LandingPresenter.Event.PresenterNavigation) },
        modifier = Modifier.padding(top = 12.dp),
      ) {
        Text(stringResource(Res.string.presenter_navigation))
      }
    }
  }
}
